...
public class DaoBoxContent {
    ...
    public boolean insert(String user_name,String box_name, String thing_name, Integer thing_num) {
        ...
        String sql = "insert into Box_Content(box_ID,thing_name,thing_num,created_time,updated_time) values((select _id from Box where box_name=? and userID=(select _id from User_Info where user_name=?)),?,?,?,?)";
        Date date = new Date();
    ...
    public boolean delete(String user_name,String box_name, String thing_name) {
        SQLiteDatabase db = mDataBaseHelper.getWritableDatabase();
        String sql =" delete from Box_Content where box_ID=(select _id from Box where box_name=? and userID=(select _id from User_Info where user_name=?)) and thing_name=?";
