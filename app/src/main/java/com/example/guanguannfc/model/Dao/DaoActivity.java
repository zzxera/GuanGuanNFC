...
public class DaoActivity {
    ...
    //向表中插入一整条数据
    public void insert(String user_ID,String nfc,String type_ID,String act_name){
    ...
    public void delete(String user_ID,String act_name){
        SQLiteDatabase db=mDataBaseHelper.getWritableDatabase();
    ...
    public void update(String user_ID,String act_oldName,String act_newName){
        SQLiteDatabase db=mDataBaseHelper.getWritableDatabase();
    ...
    public boolean query(String user_ID,String act_name){
        SQLiteDatabase db=mDataBaseHelper.getWritableDatabase();
        String sql="select * from " + GuanContract.Activity.TABLE_NAME + " where user_ID=? and act_name=?";
        Cursor cursor=db.rawQuery(sql,new String[]{user_ID,act_name});
